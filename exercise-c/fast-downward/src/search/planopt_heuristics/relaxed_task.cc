...
vector<int> RelaxedTask::translate_state(const State &state) {
    state.unpack();
    const vector<int> &unpacked_values = state.get_unpacked_values();
    
    vector<PropositionID> strips_state;
    strips_state.reserve(unpacked_values.size());
    for (size_t var = 0; var < unpacked_values.size(); ++var) {
        int value = unpacked_values[var];
